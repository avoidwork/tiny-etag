...
class ETag {
	constructor ({cacheSize = max, cacheTTL = 0, seed = random, notify = false, onchange = () => {}} = {}) {
	...
	etag (req, res, next) {
		const parsed = req.parsed || parse(req, false),
	...
	register (uri, state) {
		...
		if (newState.headers !== void 0) {
			newState.headers = Object.keys(newState.headers).filter(i => regex.invalid.test(i) === false).reduce((a, b) => {
				a[b] = state.headers[b];
